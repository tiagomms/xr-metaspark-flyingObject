...
class TrackingViz {
  constructor() {
    Promise.all([
      ...
      this.planeFound = e[2];
      D.log("--- Before init after promise resolved")
  ...
  initAfterPromiseResolved(){
    D.log("--- monitorWorldTrackingStatus")
    this.monitorWorldTrackingStatus();
    D.log("--- subscribeTrackableAdded")
    this.subscribeTrackableAdded();
    D.log("--- subscribeTrackableDeleted")
  ...
  setInstruction(enabled, token){
    D.log("+++ (1) setInstruction: " + enabled + "; " + token);
    ...
    */
    ...
    this.trackerInstruction.inputs.setBoolean('hidden', token != 'slowly_move_your_phone');
    D.log("+++ (2) setInstruction: " + enabled + "; " + token);
  }
}
